...
var DataGridRenderer = {
  ...
  xml: function (dataGrid, headerNames, headerTypes, indent, newLine) {
    ...
    outputText += "<!--Copy/Paste the below XML into dms-config.xml-->" + newLine;
    outputText += "<DataMaps>"+newLine;
    for (var i=0; i < numRows; i++) {
      var row = dataGrid[i];
      for (var d=2; d < numColumns; d++) {
        outputText += indent+'<DataMap Name="'+row[d-2]+'">'+newLine;
      };
      for (var j=1; j < numColumns; j++) {
        //Pull headerName into variable to remove whitespace and downcase the S to meet DMS config expectation.
        var elementName = headerNames[j];
        elementName = elementName.replace(/\s/g, "");
        elementName = elementName.replace("S", "s");
        outputText += indent+indent+'<'+elementName+'>';
        outputText += row[j] || ""
        outputText += '</'+elementName+'>'+newLine
      };
      outputText += indent+"</DataMap>"+newLine;
    };
    outputText += "</DataMaps>";
